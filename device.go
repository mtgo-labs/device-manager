...
// LangPack, …) that Telegram sees during initConnection. The package can
// inject them directly into a [telegram.Config] via [Profile.Apply].
//
// Profiles can be:
...
// ToDeviceConfig converts the Profile into a [telegram.DeviceConfig].
//
// Use this when building a [telegram.Config] by hand:
//
...
// Apply injects the Profile's device identity into a [telegram.Config].
// Only cfg.Device is modified; APIID and APIHash are left for the caller.
//
// Example:
//
